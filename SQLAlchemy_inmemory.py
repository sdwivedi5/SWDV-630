...
print ("For this example we will use an in-memory sqlite DB.")
...
class Address(Base):
    ...
    # Also creates FK which is accessible from a User object.
...
print ("Testing/Driver code - Let's create a user and add two e-mail addresses to that user.")
ed_user = User(name='ed', fullname='Sandeep Dwivedi', password='edspassword')
ed_user.addresses = [Address(email_address='sandeep.dwivedi@gmail.com'), Address(email_address='shield009@yahoo.com')]

# Let's add the user and its addresses we've created to the DB and commit.
session.add(ed_user)
session.commit()

print ("Testing/Driver code - Now let's query the user that has the e-mail address sandeep.dwivedi@gmail.com")
# SQLAlchemy will construct a JOIN query automatically.
user_by_email = session.query(User)\
    .filter(Address.email_address=='sandeep.dwivedi@gmail.com')\
    .first()
